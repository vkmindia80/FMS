...
**Option B: Via API**
```bash
curl -X POST "https://viewfix.preview.emergentagent.com/api/auth/generate-demo-data" \
...
1. **Navigate to Reports page**
   - URL: `https://viewfix.preview.emergentagent.com/reports`
...
```bash
curl -X POST "https://viewfix.preview.emergentagent.com/api/auth/login" \
...
```bash
curl -X GET "https://viewfix.preview.emergentagent.com/api/reports/profit-loss?period=current_month&format=json" \
...
### 3. Test Export
```bash
curl -X GET "https://viewfix.preview.emergentagent.com/api/reports/profit-loss?period=current_month&format=pdf" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE" \
